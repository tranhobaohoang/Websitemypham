	...
	<?php foreach($sp_list as $k=>$v) { ?>
		<div class="sp-box-chung">
			<div class="sp-head-box">
				<div class="sp-td sp-td-<?=$v['id']?>"><?=$v['ten']?></div>
				<div class="sp-tab-toggle sp-tab-toggle-<?=$v['id']?>">
					<?php
					$d->reset();
		...
		<?php foreach($sp_list2 as $k2=>$v2) { ?>
			$(".sp-tablink-<?=$v2['id']?>").click(function() {
				...
				$(".sp-tab-toggle-<?=$v['id']?> .sp-tablink").removeClass("active");
				$(this).addClass("active");
			});
		<?php } ?>
			$(".sp-td-<?=$v['id']?>").click(function() {
				$(".sp-tab-toggle-<?=$v['id']?>").fadeToggle();
			});
		});
		</script>
	<?php } ?>
...
<div class="tin-tuc">
	...
	<div class="container">
		...
		<div class="xem-tat-ca"><a href="tin-tuc.html">Xem tất cả >></a></div>
	</div>
</div>
