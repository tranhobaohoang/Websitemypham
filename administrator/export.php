...
$d->reset();
$sql="select * from #_dhct where id_dh = '$id_donhang'";
$d->query($sql);
...
$objPHPExcel->setActiveSheetIndex( 0 )->setCellValue( 'A'.$vitri, $i+1 )->setCellValue( 'B'.$vitri,get_product_name($items[$i]['id_sp']) )->setCellValue( 'C'.$vitri, $items[$i]['soluong'])->setCellValue( 'D'.$vitri, number_format(get_price($items[$i]['id_sp']),0,".",",") )->setCellValue( 'E'.$vitri, number_format(get_price($items[$i]['id_sp'])*$items[$i]['soluong'],0,".",",") );
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue( 'E'.$vitri,number_format(get_tong_tien($result_giohang['id']),0, '.', ','));
