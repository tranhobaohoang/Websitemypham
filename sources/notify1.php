...
$sqls="select madonhang from #_donhang where transaction='".$mTransactionID."'";
			$d->query($sqls);
			$donhang1=$d->fetch_array();
if($processResult==1){
	$sttd='Thanh toán thành công đơn hàng '.$donhang1['madonhang'].'<br/> Tin đăng của bạn sẽ được website cập nhật sau ít phút.';
}
if($processResult==2){
	$sttd='Đơn hàng đã được thanh toán trước đó!';
}
if($processResult==-3){
	$sttd='Thanh toán đơn hàng '.$donhang1['madonhang'].' thất bại';
}
/*===============================End Function region=======================================*/
?>