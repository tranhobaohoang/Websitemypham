...
function thumb($file,$path,$new='',$w=0,$h=0,$zc=1,$qty=90){
	...
	return "thumb/$w"."x"."$h/$zc/$path$name/$new.$ext";
}

function showClass($i){
	if($i==0) $kq="border1";
	else if($i<5) $kq="border2";
	else if(($i+1)%6==0) $kq="border3";
	return $kq;
}
function getTenkhongdauList($id){
	global $d;
	$d->reset();
	$sql="select tenkhongdau from #_product_list where id='".$id."'";
	$d->query($sql);
	$rs=$d->fetch_array();
	if($d->num_rows()>0){
		return $rs["tenkhongdau"];
	}else{
		return "san-pham";
	}
...
function showNameList($id){
	global $d, $lang;
	$d->reset();
	$sql="select ten_$lang as ten from #_product_list where id='".$id."'";
	$d->query($sql);
	$rs=$d->fetch_array();
	return $rs["ten"];
...
function showThuongHieu($id){
	...
	if($id>0){
	$d->reset();
	$sql="select ten_$lang as ten, tenkhongdau, id from #_thuonghieu where id='".$id."' and hienthi=1 order by stt, id desc";
	$d->query($sql);
	$rs=$d->fetch_array();
	$kq='<li><a href="thuong-hieu/'.$rs["tenkhongdau"].'-'.$rs["id"].'.html">'.$rs["ten"].'</a></li>';
	return $kq;
	}
}
...
function showItemList($id,$tenkhongdau){
	global $d, $lang;
	
	$d->reset();
	$sql="select ten_$lang as ten , tenkhongdau, id from #_product_list where find_in_set(".$id.", replace(set_level, '|', ','))>0 and com='2' and noibat=1 and hienthi=1 order by stt, id desc limit 0,4";
	$d->query($sql);;
	$rs=$d->result_array();
	$kq='<div class="list_info">';
	if(!empty($rs)){ $kq.='<ul>'; foreach($rs as $k=>$v){
		$kq.='<li><a href="'.$v["tenkhongdau"].'-'.$v["id"].'/" >'.catchuoi($v["ten"],20).'</a></li>';
	}}
	$kq.='</ul>';
	if(count($rs)==4){$kq.='<div class="readmore"><a href="'.$tenkhongdau.'-'.$id.'/">Xem thêm</a></div>';}
	$kq.='</div>';
	return $kq;
}
function day($s){
...
function get_district($province){
	global $d;
	$d->reset();
...
function export_star($star){
	$arr=explode(".",$star);
	$kq='';
	if($arr[1]!=''){
		for($i=1;$i<=5;$i++){
			if($i<$star){
				$kq.= "<img src='assets/images/star1.png' class='star' alt='star' />";
			}else if($i>$star && ($i-1)<$star){
				$kq.= "<img src='assets/images/starhapart.png' class='star' alt='star' />";
			}else{
				$kq.= "<img src='assets/images/star0.png' class='star' alt='star' />";
			}
		}
	}else{
		for($i=1;$i<=5;$i++){
...
function count_sale($gia,$giakm){
	$kq= round(($gia-$giakm)*100/$gia);
	return $kq;
}
function check_data($s) {
    echo "<pre>";
    print_r($s);
    echo "</pre>";
}
